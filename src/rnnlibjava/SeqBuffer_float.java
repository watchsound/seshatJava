...
public class SeqBuffer_float  extends MultiArray_float{

    int depth;
		 

	public SeqBuffer_float( ) { 
		depth = 0;
	}
	public SeqBuffer_float(int dep ) {
		this.depth = dep;
		reshape(new int[0]);	
	}
	public SeqBuffer_float(int[] shape, int dep ) {
		this.depth = dep;
		reshape(shape);	
	}
	public SeqBuffer_float(ResizableListI<Integer> shape, int dep ) {
		this.depth = dep;
		reshape(shape);	
	}
...
//
	  ...
	  public ViewFloat atCoord(int coord) {
	    if ((coord >= 0) && ((coord * this.shape_back()) < Helpers.productInt(this.shape))) {
		   ...
		   // const T* end = start  + this->shape.back();
		    return new ViewFloat(this.data, start, end);
	    }
	    return new ViewFloat(this.data, 0,0);
	  }
...
//
	 ...
	 public  ArrayListObj<Float> seq_means()   {
	    ...
	    for(int i = 0; i < seqSize; i++) {
	      ...
	      Helpers.range_plus_equals(seqMean,   this.atCoord(i).tov() );
	    }
	...
	//
	  ...
	  public void reshape( SeqBuffer_float   buff) {
	    reshape(buff.seq_shape());
	  }

	  public void reshape(  SeqBuffer_float  buff,  Float  fillVal) {
	    reshape(buff.seq_shape());
	    fill_data(fillVal);
	  }
	  
	  public void reshape(  View<Integer>  newSeqShape ,  Float  fillVal) {
		  reshape(newSeqShape);
		  fill_data(fillVal);
	  }
	 ...
	 public void reshape(int[] newSeqShap, Float fillval) {
	     reshape(newSeqShap);
	     fill_data(fillval);
	  }
	  ...
	  public void reshape_with_depth(int[] newSeqShap, int dep, Float fillval) { 
	  ...
	  public SeqBuffer_float assign(SeqBuffer_float a){
		  depth = a.depth;
	  ...
	  public SeqBuffer_float copy(){
		  SeqBuffer_float c = new SeqBuffer_float();
			 c.assign(this);
			 return c;
		 }
