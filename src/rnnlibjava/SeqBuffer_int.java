...
public class SeqBuffer_int  extends MultiArray_int{

    int depth;
		 

	public SeqBuffer_int( ) { 
		depth = 0;
	}
	public SeqBuffer_int(int dep ) {
		this.depth = dep;
		reshape(new int[0]);	
	}
	public SeqBuffer_int(int[] shape, int dep ) {
		this.depth = dep;
		reshape(shape);	
	}
	public SeqBuffer_int(ResizableListI<Integer> shape, int dep ) {
		this.depth = dep;
		reshape(shape);	
	}
...
//
	  ...
	  public ViewInt atCoord(int coord) {
	    if ((coord >= 0) && ((coord * this.shape_back()) < Helpers.productInt(this.shape))) {
	  	   ...
	  	   // const T* end = start  + this->shape.back();
	  	    return new ViewInt(this.data, start, end);

	    }
	    return new ViewInt(this.data, 0,0);
	  }
	...
	//	    return new Container.ViewList<Integer>(this.shape); 
	 ...
	 public  ArrayListObj<Float> seq_means()   {
	    ...
	    for(int i = 0; i < seqSize; i++) {
	      ...
	      Helpers.range_plus_equals(seqMean, Helpers.int2float( this.atCoord(i).tov()));
	    }
...
//
	  ...
	  public void reshape( SeqBuffer_int   buff) {
	    reshape(buff.seq_shape());
	  }

	  public void reshape(  SeqBuffer_int  buff,  int  fillVal) {
	    reshape(buff.seq_shape() );
	    fill_data(fillVal);
	  }
	  public void reshape(  View<Integer>  newSeqShape ,  int  fillVal) {
		  reshape(newSeqShape);
		  fill_data(fillVal);
	  }
	 ...
	 public void reshape(int[] newSeqShap, Integer fillval) {
	     reshape(newSeqShap);
	     fill_data(fillval);
	  }
	  ...
	  public void reshape_with_depth(int[] newSeqShap, int dep, int fillval) { 
	  ...
	  public SeqBuffer_int assign(SeqBuffer_int a){
		  depth = a.depth;
...
//		  resize_data();
	  ...
	  public MultiArray_int copy(){
		  SeqBuffer_int c = new SeqBuffer_int();
			 c.assign(this);
			 return c;
		 }
