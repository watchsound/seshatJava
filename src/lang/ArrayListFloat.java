...
public class ArrayListFloat extends AbstractList<Float> implements List<Float>, ResizableListI<Float>,RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 * @return
	 */
	public static boolean maybeNull(float value) {
		return value == NULL;
	}
	 ...
	 */
	transient float[] elementData; // non-private to simplify nested class access
	...
	public static ArrayListFloat create(int size, float fill) {
		ArrayListFloat obj = new ArrayListFloat(size);
	...
	*/
	public ArrayListFloat(Collection<? extends Float> c) {
		Float[] cc = c.toArray(new Float[0]);
		for(int i = 0; i < c.size(); i++)
			elementData[i] =  cc[i];
	 
	if ((size = elementData.length) != 0) {
	    // c.toArray might (incorrectly) not return Object[] (see 6260652)
	    if (elementData.getClass() !=  float[].class)
    ...
    public ArrayListFloat(float[] data){ 
    	this();
		this.addAll(data);
	}
    
    public ArrayListFloat copy(){
    	ArrayListFloat c1 = new ArrayListFloat( );
    	 
		for(int i = 0; i < elementData.length ; i++)
			c1.elementData[i] =  elementData[i];
...
// minCapacity is usually close to size, so this is a win:
	...
	public Object[] toArray() {
		Object[] v = new Float[size];
		for(int i = 0; i < size; i++)
			v[i] = elementData[i];
		return v;
	}
	public float[] toArrayPrimary() {
	...
	public float[] toArrayPrimary(float[] a) {
		if (a.length < size)
...
// Positional Access Operations
	...
	public boolean remove(Float o) {
		return removePrimary(o.floatValue());
	}
	public boolean removePrimary(float o) {
		if (maybeNull(o)) {
...
// clear to let GC do its work
	...
	public static final boolean contains(float[] c, float v) {
		for(float i : c) {
			if( i == v)
				return true;
		}
	...
	public static final int indexOf(float[] c, float v) {
	...
	private boolean batchRemove(float[] c, boolean complement) {
		final float[] elementData = this.elementData;
		int r = 0, w = 0;
		boolean modified = false;
		try {
...
// Read in capacity
	...
	private class Itr implements Iterator<Float> {
		...
		public Float next() {
			...
			float[] elementData = ArrayListFloat.this.elementData;
		...
		public void remove() {
			...
			try {
				ArrayListFloat.this.remove(lastRet);
				cursor = lastRet;
				lastRet = -1;
		...
		public void forEachRemaining(Consumer<? super Float> consumer) {
			...
			final int size = ArrayListFloat.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final float[] elementData = ArrayListFloat.this.elementData;
			if (i >= elementData.length) {
				throw new ConcurrentModificationException();
			}
			while (i != size && modCount == expectedModCount) {
				consumer.accept((Float) elementData[i++]);
			}
	...
	private class ListItr extends Itr implements ListIterator<Float> {
		...
		public void set(Float e) {
			...
			try {
				ArrayListFloat.this.set(lastRet, e);
			} catch (IndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		}
	 ...
	 */
	public ArrayListFloat subList(int fromIndex, int toIndex) {
	...
	private class SubList extends ArrayListFloat implements RandomAccess {
		private final ArrayListFloat parent;
		private final int parentOffset;
		private final int offset;
		int size;

		SubList(ArrayListFloat parent, int offset, int fromIndex, int toIndex) {
			this.parent = parent;
			this.parentOffset = fromIndex;
			this.offset = offset + fromIndex;
			this.size = toIndex - fromIndex;
			this.modCount = ArrayListFloat.this.modCount;
		}

		public float set(int index, int e) {
			rangeCheck(index);
			checkForComodification();
			float  oldValue = ArrayListFloat.this.elementData(offset + index);
			ArrayListFloat.this.elementData[offset + index] = e;
			return oldValue;
		}
		...
		public Float get(int index) {
			rangeCheck(index);
			checkForComodification();
			return ArrayListFloat.this.elementData(offset + index);
		}
		...
		public ListIterator<Float> listIterator(final int index) {
			...
			return new ListIterator<Float>() {
				int cursor = index;
				int lastRet = -1;
				int expectedModCount = ArrayListFloat.this.modCount;
				...
				public void remove() {
					...
					try {
						...
						cursor = lastRet;
						lastRet = -1;
						expectedModCount = ArrayListFloat.this.modCount;
					} catch (IndexOutOfBoundsException ex) {
						throw new ConcurrentModificationException();
					}
				}
				...
				public void set(Float e) {
					...
					try {
						ArrayListFloat.this.set(offset + lastRet, e);
					} catch (IndexOutOfBoundsException ex) {
						throw new ConcurrentModificationException();
					}
				}
				...
				final void checkForComodification() {
					if (expectedModCount != ArrayListFloat.this.modCount)
						throw new ConcurrentModificationException();
				}
			};
		}
		...
		private void checkForComodification() {
			if (ArrayListFloat.this.modCount != this.modCount)
				throw new ConcurrentModificationException();
		}

		public Spliterator<Float> spliterator() {
			checkForComodification();
			return new ArrayListSpliterator(ArrayListFloat.this, offset, offset + this.size, this.modCount);
		}
	}
	...
	static final class ArrayListSpliterator  implements Spliterator  {
		...
		private final ArrayListFloat list;
		private int index; // current index, modified on advance/split
		private int fence; // -1 until used; then one past last index
		private int expectedModCount; // initialized when fence set

		/** Create new spliterator covering the given range */
		ArrayListSpliterator(ArrayListFloat list, int origin, int fence, int expectedModCount) {
...
// will leave the collection unmodified
		...
		for (int i = 0; modCount == expectedModCount && i < size; i++) {
			@SuppressWarnings("unchecked")
			final float element =   elementData[i];
